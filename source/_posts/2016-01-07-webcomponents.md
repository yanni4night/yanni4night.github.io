...
<script>
  ...
  document
    .querySelector('#rank')
    .insertBefore(template.content, document.querySelector('#rank').lastChild)
</script>
```
...
值得一提的是，Polymer 支持更复杂的 template，比如 **mustache** 语法及 _dom-if_、_dom-repeat_ 指令，有点类似于 Angular 的 _ng-if_ 和 _ng-repeat_。
...
将 `Custom Elements` 作为 **组件名** 和 **组件引用指令**，如：

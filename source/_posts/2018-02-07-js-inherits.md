...
在很多年前，jQuery 作者 _John Resig_ 曾写过一个继承的[实现](https://johnresig.com/blog/simple-javascript-inheritance/)，在那个还在 ES3 语法的时代已经非常难得。在今天看来，这种实现方式的语义已经远远落后，同时也不能实现静态变量。
...
想实现 B 继承于 A，那边必然 _b instanceof B_ 为真，根据 instanceof 的意义，一定有：
...
于是我们覆写 _B.prototype_：

```js
B.prototype = Object.create(A.prototype)
```

这样一定有 _A.prototype.isPrototypeOf(B.prototype)_ 为真。
...
在 B 环境中，灵活的做法是通过 B.prototype 找到 A.prototype，根据 _B.prototype = Object.create(A.prototype)_，我们知道：
...
最后我们来实现在构造方法中调用 super()。显然，实质是在 B 函数中找到函数 A，根据 _Object.setPrototypeOf(B, A)_，我们有：
...
下图很好地反应了诸多对象的关系。值得一提的是， _b_ 也可能成为下一级类的 prototype，你可以清晰地看到通过 \_\_proto\_\_ 搜索原型链的轨迹。

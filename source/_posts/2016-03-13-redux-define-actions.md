...
在 Redux 中，我们只需要在 store(state) 中定义最原始的状态数据即可，\_对该数据任何一部分的改变即可定义为一个Action\_\_，而将状态数据转化成视图数据的逻辑可以定义在 `mapStateToProps` 函数中，这是一个 [redux-react](https://github.com/reactjs/react-redux) 中的函数。总之，视图数据应该和状态数据保持分离，并维持状态数据的简洁性，从而使得 Action 的定义更清晰。
